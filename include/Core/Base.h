...
namespace BnB {
    ...
    // shall be specified by the user, constants will be initialized once
    ...
    public:
        ...
        std::function<std::vector<Subproblem_Params>(const Problem_Consts &,
                                                     const Subproblem_Params &)> SplitSolution = nullptr;
        ...
        std::function<std::tuple<Domain_Type, Domain_Type>(const Problem_Consts &,
                                                           const Subproblem_Params &)> GetEstimateForBounds = nullptr;
    ...
    // function used by all solvers, takes their queue and returns the next task
    ...
    Subproblem_Params GetNextSubproblem(std::deque<Subproblem_Params> &Container, TraversalMode Mode) {
        ...
        switch (Mode) {
            case TraversalMode::DFS:
                NextSubproblem = Container.back();
                Container.pop_back();
                break;
            case TraversalMode::BFS:
                NextSubproblem = Container.front();
                Container.pop_front();
                break;
